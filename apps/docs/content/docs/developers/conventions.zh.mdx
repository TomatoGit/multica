...
项目文档和页面默认只维护中文，并以中文为第一语言；只有任务明确要求时才新增或同步其他语言版本。
...
修改保留 slug 时，同时更新 `server/internal/handler/reserved_slugs.json` 和生成文件 `packages/core/paths/reserved-slugs.ts`。
...
- handler 里的 UUID 解析规则见 `docs/agents/backend.md`
...
**只允许中文**。Go 和 TypeScript 都遵循此规则；发现英文代码注释时应改为中文。
...
1. 默认只更新中文 locale JSON 和中文 docs 页面；明确要求翻译时再同步其他语言。影响智能体行为时，更新对应的 `docs/agents/*.md`
