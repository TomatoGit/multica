...
예약 slug를 변경할 때는 `server/internal/handler/reserved_slugs.json`과 생성 파일 `packages/core/paths/reserved-slugs.ts`를 함께 업데이트하세요.
...
- handler의 UUID 파싱 규칙은 `docs/agents/backend.md`를 참고하세요.
...
중국어만 사용합니다. Go와 TypeScript 모두에 적용됩니다. 영어 코드 주석은 중국어로 번역하세요.
...
1. 관련 locale JSON과 문서 페이지에 적용하고, 에이전트 동작에 영향을 주는 경우 해당 `docs/agents/*.md`도 업데이트
