...
予約済み slug を変更する場合は、`server/internal/handler/reserved_slugs.json` と生成ファイル `packages/core/paths/reserved-slugs.ts` を同時に更新してください。
...
- handler の UUID 解析ルールは `docs/agents/backend.md` を参照してください。
...
中国語のみです。Go と TypeScript の両方に適用されます。英語のコードコメントは中国語に翻訳してください。
...
1. 関連する locale JSON とドキュメントページに適用する。エージェントの動作に影響する場合は、対応する `docs/agents/*.md` も更新する
