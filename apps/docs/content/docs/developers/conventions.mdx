...
When changing reserved slugs, update both `server/internal/handler/reserved_slugs.json` and the generated `packages/core/paths/reserved-slugs.ts`.
...
- For UUID parsing in handlers, follow `docs/agents/backend.md`.
...
Chinese only. This applies to both Go and TypeScript; translate English code comments into Chinese.
...
1. Apply it in the relevant locale JSONs and docs pages; if it changes agent behavior, update the corresponding `docs/agents/*.md`
